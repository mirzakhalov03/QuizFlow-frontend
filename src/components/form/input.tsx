...
export function FormInput<IForm extends FieldValues>({
  ...
  return (
    <fieldset className={cn('flex w-full flex-col justify-between', wrapperClassName)}>
      {label && (
        <FieldLabel
          htmlFor={name}
          required={required}
      ...
      <Input
        ...
        suffix={suffix}
        className={cn(
          error ? 'focus:border-border border-red-600 ring-red-600!' : '',
          uppercase && 'uppercase placeholder:capitalize',
          className
        )}
      />
      {!hideError && error?.message && <FieldError>{error?.message as string}</FieldError>}
    </fieldset>
  )
}
